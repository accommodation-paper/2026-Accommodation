...
from accommodation.experiments.runners.frozen_backbone.text_model_runner import run_experiment
from accommodation.model.set_seed import set_seed


def run_frozen_backbone_imdb_experiment(args):
	set_seed(42)
	dataset = IMDBDataset(args["data-path"])
	train_size = int(0.8 * len(dataset))
	val_size = len(dataset) - train_size
	train_ds, val_ds = random_split(dataset, [train_size, val_size], generator=torch.Generator().manual_seed(42))
	train_loader = DataLoader(train_ds, batch_size=64, shuffle=True)
	val_loader = DataLoader(val_ds, batch_size=64, shuffle=False)
	run_experiment("IMDB", args, dataset.vocab_size, train_loader, val_loader, instantiate_gru_linear_classifier, instantiate_gru_accommodation_classifier)
