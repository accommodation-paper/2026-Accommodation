...
def run_experiment(dataset: str, args: dict, vocab_size: int, train_loader: DataLoader, val_loader: DataLoader, instantiate_model: Callable):
    for cycle in range(args["num-cycles"]):
        ...
        print(f"[{dataset}] Cycle {cycle + 1:02d}/{args['num-cycles']:02d} | seed={seed} | building model on {args['device']}", flush=True)
        ...
        print(f"Cycle {cycle + 1:02d} Done | Results saved on: {out_path}", flush=True)
