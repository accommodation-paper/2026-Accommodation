...
def instantiate_gru_accommodation_classifier(args, vocab_size):
    return GRUAccommodationClassifier(
        vocab_size=vocab_size,
        in_features=args['embedding-dim'],
        num_classes=args['num-classes'],
        latent_dim=args['latent-dim'],
        negative_potents=args['negative-potents'],
        neutral_potents=args['neutral-potents'],
        plasticity=args["plasticity"],
        num_potents_per_class=args["num-potents-per-class"],
    )
